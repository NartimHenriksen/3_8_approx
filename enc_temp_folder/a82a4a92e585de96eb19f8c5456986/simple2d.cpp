...
string make_strand(vector<bool> seq, int pairings, int j, bool wrap, string udlrfb) {
	...
	while (pairings > 0) {
		...
		if (layercounter < j) { //normal loop
		...
		else { //time for layer stuff. iter doesnt matter here
			...
			if (n - 1 - res.size() < 8) {
				if (wrap) {
					...
					wrap.erase(n - 1 - res.length(), 8 - (n - 1 - res.length()));
					res += wrap;
					return(res);
				}
				else {
					...
					string tunnel(arr);
					if (n - 1 - res.length() < 4) {
						tunnel.erase((n - 1 - res.length(), 4 - (n - 1 - res.length())));
						res += tunnel;

						int leftovers = (n - 1 - res.length());
						if (leftovers > 0) {
							res += string(leftovers, inc);//spill the rest
						}
						return(res);
					}
					res += tunnel;
			...
			//otherwise do a normal wrap or tunnel

			// add all 8, set iter and lasth and pairings and reset layercounter and wrap
			...
			if (wrap) {
				...
				//subtract pairings that are lost int the wrap
				int skipped = seq[lasth + 2] + seq[lasth + 4] + seq[lasth + 6]; //2 wrap slots and 1 blind spot
				pairings -= skipped;


				char arr[9] = {forw,dec,dec,back,udlrfb[4],'\0'};
				string wrap(arr);
				//string wraap(forw, dec, dec, back, udlrfb[4]);

				int foldgap=0;

				if(pairings>0){//if there are more h left
					while (!seq[lasth + 8 + 2 * foldgap]) { foldgap++; }//find the next one to land on					
					pairings -= 1; //for the first one in next row
				}
				//else leave foldgap at 0 to go back to layer and it will handle the tail end. We can do this because we checked string has 8 chars left


				wrap += string(foldgap, dec) + back + string(foldgap, inc) + inc + back;
				res += wrap;


				iter = lasth + 8 + 2 * foldgap;
				lasth = iter;
				if (iter != res.size()) { cout << "mb check this out" << endl; }
			...
			//do the "tunnel"
				...
				int skipped = seq[lasth + 2];
				pairings -= skipped; //if blind spot was h


				char arr[5] = { udlrfb[4],dec,'\0' };
				string tunnel(arr);

				int foldgap = 0;
				//if there are more h
				if (pairings > 0) {
					while (!seq[lasth + 4 + 2 * foldgap]) { foldgap++; }//find h to land on
					pairings -= 1; //from the first h in new line
				}
				//else leave foldgap 0 to return to layer
				tunnel += string(foldgap, dec) + back + string(foldgap, inc) + back;
				res += tunnel;

				//cout << "skipped: " << skipped;

				iter = lasth + 4+2*foldgap;
				lasth = iter;
				if (iter != res.size()) { cout << "mb check this out" << endl; }
	...
	//if we are missing a piece then it must be in the middle of a layer (wrap/tunnel should be handled in loop), so we can freely extend in INC direction
	int missing = n - 1 - res.size();
	//return res+string(missing,inc);
	return res + string(missing, inc);
}
